...
vector<int> inorderTraversal(TreeNode* root) {
  ...
  while (!inorder.empty()) {
    ...
    if (nullptr != tmp.right) {
      ...
      while (nullptr != tmp_right.left) {
        inorder.push(tmp_right);
...
int main(int argc, char* argv[]) {
  ...
  inorderTraversalRecursive(tree1_root);
  cout << endl;
  
  cout << "inorder traversal tree(no recursive): ";
  tree1_root = NULL;
  vector<int> inorderTraversalRes = inorderTraversal(tree1_root); 
  for(vector<int>::iterator it = inorderTraversalRes.begin(); it != inorderTraversalRes.end(); ++it) {
    cout << *it;
  }
  cout << endl;
  return 0;

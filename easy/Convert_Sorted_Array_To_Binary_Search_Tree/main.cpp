...
vector<int> splitToInt(string str, char a) {
  vector<int> strvec;
  string::size_type pos1, pos2;
  pos2 = str.find(a);
  pos1 = 0;
...
TreeNode* buildBST(vector<int>& nums, int low, int high, TreeNode* root) {
  if (low <= high) {
    int mid = (low + high) / 2;
...
TreeNode* sortedArrayToBST(vector<int>& nums) {
  ...
  return buildBST(nums, low, high, NULL);
}
...
int main(int argc, char* argv[]) {
  ...
  vector<int> list = splitToInt(FLAGS_tree1_preorder, ',');

  TreeNode *tree1_root = sortedArrayToBST(list);

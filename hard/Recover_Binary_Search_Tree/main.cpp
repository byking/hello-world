...
void inorderTraversalRecord(TreeNode* node, vector<TreeNode*> &nodeList, vector<int> &valList) {
  if (nullptr == node) return;
  if (nullptr != node->left) {
    inorderTraversalRecord(node->left, nodeList, valList);
  }
  nodeList.push_back(node);
  valList.push_back(node->val);
  if (nullptr != node->right) {
    inorderTraversalRecord(node->right, nodeList, valList);
  }
}
...
void recoverTree(TreeNode* root) {
  ...
  vector<int> valList;
  inorderTraversalRecord(root, nodeList, valList);  
  sort(valList.begin(), valList.end());
  for (int i = 0; i < static_cast<int>(valList.size()); i++) {
